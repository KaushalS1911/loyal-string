...
const TABLE_HEAD = [
  ...
  { id: 'Department Name', label: 'Department Name' },
  { id: 'Department Code', label: 'Department Code' },
  { id: 'Department Head', label: 'Department Head' },
  { id: 'Department Description', label: 'Department Description' },
  { id: '', width: 88 },
];
...
export default function DepartmentListView() {
  ...
  const handleDelete = async (id) => {
    try {
      const res = await axios.delete(`${ASSETS_API}/api/company/${user?.company}/department`, {
        data: { ids: id },
      });
  ...
  const handleDeleteRows = useCallback(() => {
    const deleteRows = department.filter((row) => table.selected.includes(row._id));
